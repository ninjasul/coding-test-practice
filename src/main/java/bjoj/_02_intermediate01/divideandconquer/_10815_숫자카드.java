...
2 초	    256 MB	    15033	7132	4773	    47.025%
...
public class _10815_숫자카드 {
    public static void main(String[] args) {
        ...
        Set<Integer> cards = getNumberSet(sc, numberOfCards);

        int numberOfTargets = Integer.parseInt(sc.nextLine());
        Set<Integer> targets = getNumberSet(sc, numberOfTargets);

        for (int target : targets) {
            System.out.print(cards.contains(target) ? "1 " : "0 ");
        }
        System.out.println();
    }

    private static Set<Integer> getNumberSet(Scanner sc, int number) {
