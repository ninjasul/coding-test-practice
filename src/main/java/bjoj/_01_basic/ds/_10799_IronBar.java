...
public class _10799_IronBar {
	public static void main(String[] args) {
		...
		int ironBarCount = 0;
		int ironBarSum = 0;
		boolean wasPrevOpening = false;

		for( char curChar : cmdLine.toCharArray() ) {
			if( curChar == '(' ) {
				if( wasPrevOpening ) {
					ironBarCount++;
					ironBarSum++;
				...
				if( wasPrevOpening ) {
					ironBarSum += ironBarCount;
				}
				else {
					ironBarCount--;
				}

				wasPrevOpening = false;
			}
		}

		System.out.println(ironBarSum);
	}
