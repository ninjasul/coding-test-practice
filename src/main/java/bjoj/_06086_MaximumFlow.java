...
두개의 배수관이 한줄로 연결 돼 있을 때 두 관의 유량 중 최소값으로 흐르게 된다. 
예를 들어 용량이 5인 파이프가 용량이 3인 파이프와 연결되면 한개의 용량 3짜리 파이프가 된다.
...
한 파이프들의 집합을 읽고. 두개의 끝점을 가진 파이프로 만들어놓은 뒤 
A부터 Z까지 흐르는 최대 유량을 계산하라. 
모든 파이프들은 위의 규칙을 적용시켜 줄일 수 있다.

i번째 파이프는 두개의 다른 노드 a_i와 b_i(a_i in range 'A-Za-z'; b_i in range 'A-Za-z')와 연결돼 있고 
F_i(1<=F_i<=1,000)만큼의 유량을 갖는다. 
대문자와 소문자로 구성된 이름은 서로 다른 문자로 인식한다.

입력
첫째 줄에 정수 N (1 ≤ N ≤ 700)이 주어진다. 
둘째 줄부터 N+1번째 줄까지 파이프의 정보가 주어진다. 
첫 번째, 두 번째 위치에 파이프의 이름(알파벳)이 주어지고, 
...
3
 */
public class _06086_MaximumFlow {
	
	static private List<List<Edge>> graph;
