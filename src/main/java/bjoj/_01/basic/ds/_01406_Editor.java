...
public class _01406_Editor {
    public static void main(String[] args) throws IOException {
        ...
        for( int i = 0; i < cmdLength; ++i ) {
            ...
            // backspace
            else if (cmds[0].equals("B")) {
                if (leftStack.size() > 0) {
                    leftStack.pop();
                }
            }
            // add a character to the left
            else if (cmds[0].equals("P")) {
                leftStack.push(cmds[1].charAt(0));
            }
        }

        StringBuilder sb = new StringBuilder();
        for( int i = 0, length = leftStack.size(); i < length; ++i ) {
